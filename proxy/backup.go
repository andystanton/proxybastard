...
// DirtyRestore does a restore by using reflection over the struct. Filthy.
func DirtyRestore(config Configuration) {
			...
			if fieldName == "Files" {
				...
				for _, file := range files {
					...
					if userHomeRegex.MatchString(sanitisedFile) {
						...
						fmt.Printf("copying %s to %s\n", fmt.Sprintf("%s/%s", fileBackupPath, fileName), sanitisedFile)
